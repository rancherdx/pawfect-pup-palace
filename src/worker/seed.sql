...
INSERT OR IGNORE INTO puppies (id, name, litter_id, breed_id, gender, color, birth_date, weight, price, status, microchip_id, description, temperament_notes, health_notes, image_urls, created_at, updated_at)
VALUES
  ('puppy-001', 'Buddy', 'litter-001', 'breed-001', 'Male', 'Yellow', '2025-06-15', 3.2, 2500.00, 'available', 'mc001', 'Playful and energetic yellow lab puppy, loves to fetch.', 'Very outgoing, curious.', 'First shots complete, dewormed.', '["https://images.unsplash.com/photo-1587764379873-97837921fd44?ixlib=rb-4.0.3"]', STRFTIME('%Y-%m-%d %H:%M:%f', 'now'), STRFTIME('%Y-%m-%d %H:%M:%f', 'now')),
  ('puppy-002', 'Lucy', 'litter-001', 'breed-001', 'Female', 'Black', '2025-06-15', 2.9, 2500.00, 'available', 'mc002', 'Sweet and calm black lab puppy, loves cuddles.', 'Gentle, observant.', 'First shots complete, dewormed.', '["https://images.unsplash.com/photo-1518020382113-a7e8fc38eac9?ixlib=rb-4.0.3"]', STRFTIME('%Y-%m-%d %H:%M:%f', 'now'), STRFTIME('%Y-%m-%d %H:%M:%f', 'now')),
  ('puppy-003', 'Rex', 'litter-001', 'breed-001', 'Male', 'Black', '2025-06-15', 3.5, 2600.00, 'reserved', 'mc003', 'Confident and curious black lab puppy, very intelligent.', 'Bold, quick learner.', 'First shots complete, dewormed. Vet check clear.', '["https://images.unsplash.com/photo-1505628346881-b72b27e84530?ixlib=rb-4.0.3"]', STRFTIME('%Y-%m-%d %H:%M:%f', 'now'), STRFTIME('%Y-%m-%d %H:%M:%f', 'now')),
  ('puppy-004', 'Goldie', 'litter-002', 'breed-002', 'Female', 'Golden', '2025-03-20', 3.0, 2700.00, 'available', 'mc004', 'Classic golden retriever charm, very affectionate.', 'Sweet-natured, loves people.', 'Vet checked, first vaccinations.', '["https://images.unsplash.com/photo-1568640347023-a616a30bc3bd?ixlib=rb-4.0.3"]', STRFTIME('%Y-%m-%d %H:%M:%f', 'now'), STRFTIME('%Y-%m-%d %H:%M:%f', 'now'));
...
-- Note: 'password' column in users table was renamed to 'password_hash' in schema.sql
INSERT OR IGNORE INTO users (id, email, name, password_hash, roles, created_at, updated_at, phone, address, preferences)
VALUES
...
-- Ensure `users` table uses `password_hash`
-- This is handled by the schema alignment; the seed data already uses `password_hash`.

-- Ensure `puppies` table has `breed_id`
-- This is handled by the schema alignment; the seed data already includes `breed_id`.

-- Ensure `image_urls` are seeded as JSON strings
-- This is already the case in the master_seed.sql.
-- Example: '["url1", "url2"]' for puppies.image_urls
-- Example: '{"name": "Doc Name", "url": "doc_url"}' for puppy_health_records.details when type is 'document'

-- Ensure user data includes new fields (phone, address, preferences)
-- This is already the case in the master_seed.sql for the `users` table.

-- Ensure foreign key relationships are correctly defined and seeded.
-- Master seed data is structured to respect FK constraints (e.g., breeds before litters, litters before puppies).
-- `ON DELETE SET NULL` or `ON DELETE CASCADE` are used appropriately in the schema.
-- `INSERT OR IGNORE` is used to prevent errors if records already exist, which is useful for re-runnable seeding.
-- However, for critical FKs like `puppies.litter_id` referencing `litters.id`, if a litter is not seeded, the puppy insert would fail if not for `ON DELETE SET NULL` or if the ID is mandatory.
-- The current master_seed.sql ensures parent records are created first.
-- The `master_schema.sql` uses `REFERENCES breeds(id) ON DELETE SET NULL` for `litters.breed_id` and `puppies.breed_id` which is good.
-- `puppies.litter_id REFERENCES litters(id) ON DELETE CASCADE` is also good.
-- `adoptions.puppy_id REFERENCES puppies(id) ON DELETE RESTRICT` is a strong constraint ensuring adopted puppies are not deleted.
-- `conversations.user_id REFERENCES users(id) ON DELETE CASCADE` means messages will be deleted if user is deleted. This might be intended.
-- `messages.conversation_id REFERENCES conversations(id) ON DELETE CASCADE` is standard.
-- `stud_dogs.breed_id REFERENCES breeds(id) ON DELETE SET NULL` is fine.
-- `stud_dogs.owner_user_id REFERENCES users(id) ON DELETE SET NULL` is fine.

-- Add new email template for data deletion requests
INSERT OR IGNORE INTO email_templates (id, template_name, subject_template, html_body_template, is_system_template, created_at, updated_at)
VALUES ('admin_data_deletion_notification', 'Admin Data Deletion Notification', 'New User Data Deletion Request (ID: {{requestId}})',
        '<p>A new user data deletion request has been submitted.</p><ul><li>Request ID: {{requestId}}</li><li>Name: {{name}}</li><li>Email: {{email}}</li><li>Account Creation Timeframe: {{account_creation_timeframe}}</li><li>Puppy IDs: {{puppy_ids}}</li><li>Details: {{additional_details}}</li></ul><p>Please review this request in the admin dashboard: <a href="{{admin_link}}">Manage Request</a></p>',
        TRUE, STRFTIME('%Y-%m-%d %H:%M:%f', 'now'), STRFTIME('%Y-%m-%d %H:%M:%f', 'now'));
