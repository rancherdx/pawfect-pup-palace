...
create policy if not exists "admin_select_integrations"
  on public.third_party_integrations
  for select
  to authenticated
  using (public.has_role(auth.uid(), 'admin'));

create policy if not exists "admin_insert_integrations"
  on public.third_party_integrations
  for insert
  to authenticated
  with check (public.has_role(auth.uid(), 'admin'));

create policy if not exists "admin_update_integrations"
  on public.third_party_integrations
  for update
  to authenticated
  using (public.has_role(auth.uid(), 'admin'))
...
create policy if not exists "admin_delete_integrations"
  on public.third_party_integrations
  for delete
  to authenticated
  using (public.has_role(auth.uid(), 'admin'));
...
create policy if not exists "admin_select_change_logs"
  on public.change_logs
  for select
  to authenticated
  using (public.has_role(auth.uid(), 'admin'));

create policy if not exists "admin_insert_change_logs"
  on public.change_logs
  for insert
  to authenticated
  with check (public.has_role(auth.uid(), 'admin'));
...
create policy if not exists "admin_select_apple_pay"
  on storage.objects
  for select
  to authenticated
  using (bucket_id = 'apple-pay' and public.has_role(auth.uid(), 'admin'));

create policy if not exists "admin_insert_apple_pay"
  on storage.objects
  for insert
  to authenticated
  with check (bucket_id = 'apple-pay' and public.has_role(auth.uid(), 'admin'));

create policy if not exists "admin_update_apple_pay"
  on storage.objects
  for update
  to authenticated
  using (bucket_id = 'apple-pay' and public.has_role(auth.uid(), 'admin'))
...
create policy if not exists "admin_delete_apple_pay"
  on storage.objects
  for delete
  to authenticated
  using (bucket_id = 'apple-pay' and public.has_role(auth.uid(), 'admin'));
