-- Phase 2: Fix Database Relationships & Add Breed Templates (Fixed)
...
-- Only insert breed templates that don't already exist
...
) AS v(breed_name, description, size, temperament, average_weight_min, average_weight_max, life_expectancy_min, life_expectancy_max, good_with_kids, good_with_pets, hypoallergenic, exercise_needs, grooming_needs, health_considerations, care_instructions, common_traits)
WHERE NOT EXISTS (
  SELECT 1 FROM breed_templates WHERE breed_templates.breed_name = v.breed_name
);
